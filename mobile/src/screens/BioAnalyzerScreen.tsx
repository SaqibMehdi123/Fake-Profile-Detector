...
export default function BioAnalyzerScreen() {
  const add = useStore((s) => s.add);
  const [text, setText] = useState('');
  const [result, setResult] = useState<(AnalyzeResult & { text: string }) | null>(null);
  ...
  const submit = async () => {
    ...
    setResult(null);
    try {
      const r = await api.analyzeBio(text);
      setResult(r);
      ...
      await add({
        ...
        result: {
          ...
          risk_level: r.risk_level,
          reasons: r.reasons,
          notes: ['Bio heuristic analysis only'],
        } as any,
      });
    } catch (e: any) {
    ...
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: colors.bg }} behavior={RNPlatform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.intro}>
          <Text style={styles.title}>Bio / Text Analyzer</Text>
          <Text style={styles.sub}>Paste any profile bio. Detects spam keywords, link density, and bot-style patterns.</Text>
        </View>
        ...
        <TextInput
          ...
          placeholder="Paste profile bio here..."
          multiline
          numberOfLines={5}
          outlineColor={colors.border}
          activeOutlineColor={colors.primary}
          textColor={colors.text}
          style={{ backgroundColor: colors.bgSurface, minHeight: 130 }}
        />
        ...
        {result && (
          <View style={{ marginTop: 22 }}>
            ...
            <View style={{ marginTop: 14 }}>
              <ReasonList reasons={result.reasons} title="Detected signals" />
            </View>
          </View>
        )}
      </ScrollView>
